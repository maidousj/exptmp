...
leak = {}
for c,row in enumerate(csv.DictReader(open('/data/input/promoted_content.csv'))):
    if row['document_id'] != '':
        leak[row['document_id']] = 1 
print(len(leak))
count = 0
filename = '/data/input/page_views.csv'
#filename = '/data./input/page_views_sample.csv' # comment this out locally
